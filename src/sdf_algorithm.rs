...
fn calculate_sdf_at_rec(
    mm: &Arc<Mipmap>,
    x: u32,
    y: u32,
    dst_level: u8,
    best_dst_sqr_input: DstT,
    task: &SdfTask,
    needed: u8,
    pxpos: &UniPoint,
) -> DstT {
    ...
    if new_level == 0 {
        ...
        let mut child_tasks = [SdfTask {
            x: 0,
            y: 0,
            level: 0,
            best_case_dst_sqr: 0f64,
        }; 4];
        ...
        for tup in children.iter() {
            ...
            if has_needed(mmget(mm, cx, cy, new_level), needed) {
                ...
                if mindstsqr < best_dst_sqr {
                    child_tasks[child_tasks_idx] = SdfTask {
                        x: cx,
                        y: cy,
                        level: new_level,
                        best_case_dst_sqr: mindstsqr,
        ...
        child_tasks_slice.sort_by(|a: &SdfTask, b: &SdfTask| -> cmp::Ordering {
            a.best_case_dst_sqr
                .partial_cmp(&b.best_case_dst_sqr)
                .unwrap()
        });
        for child_task in child_tasks_slice {
            if child_task.best_case_dst_sqr < best_dst_sqr {
                best_dst_sqr = calculate_sdf_at_rec(
                    mm,
                    x,
                    y,
                    dst_level,
...
fn calculate_sdf_at(mm: &Arc<Mipmap>, x: u32, y: u32, dst_level: u8) -> f64 {
    ...
    let best_dst_sqr = calculate_sdf_at_rec(
        mm,
        x,
        y,
        dst_level,
        f32::INFINITY as DstT,
        &task,
        get_needed(pxval),
        &pxpos,
    );
...
pub fn calculate_sdf(mm: Arc<Mipmap>, size: u32) -> Box<SDFImage> {
    {
        let coarsest_val = mm
            .images
            .last()
            .expect("Mipmap had no images at all")
            .get_pixel(0, 0)[0];
        if !has_black_and_white(coarsest_val) {
            // image has only one color -> everywhere is the maximum distance
            debug_assert!(
                has_black(coarsest_val) || has_white(coarsest_val),
                "Mipmap is wrong: Image seems to have neither black nor white pixels"
            );
    ...
    let dst_level =
        mm.get_max_level() - log2(size as u64).expect("destination size must be a power of two");
